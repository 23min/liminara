...
**Status:** superseded by D-2026-04-26-034
**Context:** E-21 (Pack Contribution Contract) is structured as an umbrella epic composed of four sub-epics (E-24 through E-27), each with its own spec file, `parent: E-21-pack-contribution-contract` frontmatter, and `composed_of:` listing on the umbrella. This predates the 2026-04-22 framework bump, which codifies a flat epic shape (`id`, `status`, `depends_on`, `completed`) with milestones as the next-level-down unit. The umbrella pattern (`parent`, `composed_of`, `phase` frontmatter fields) is not in the framework `epic-spec.md` template and not prescribed anywhere in `.ai-repo/`.
...
## D-2026-04-24-031: Doc-tree reorganized along bind-me / inform-me taxonomy
**Status:** active
**Context:** `migrate.sh §3.1e` flagging three unset path fields (`researchPath` / `architecturePath` / `specsPath`) seeded a review that exposed three interconnected issues in the `docs/` tree: (1) the word "spec" is used for three different artifact types without a shared location discipline, (2) `docs/architecture/contracts/` is a misnamed grouping that bundles policies, indexes, and future schemas under one "contracts" label and buries them under `architecture/`, and (3) the repo has no articulated distinction between artifacts the AI must *obey* when doing work and artifacts the AI *reads for context* when iterating on design. E-21 (Pack Contribution Contract) is about to generate ~14 ADRs + 14 CUE schemas + fixtures + policies that would all inherit the legacy `docs/architecture/contracts/` path; fixing the structure before that content lands is substantially cheaper than rerouting afterward.
**Decision:** Adopt the bind-me / inform-me register distinction as the organizing principle of `docs/`. Full taxonomy, all sub-decisions, and every considered alternative are recorded in [ADR-0003](../docs/decisions/0003-doc-tree-taxonomy.md). Executed as epic **E-22-docs-foundation** with two milestones:
- **M-DOCS-01** (framework prep): PR on the `.ai/` submodule (`ai-workflow` repo) removing `specsPath` from paths / skills / sync / migrate and softening `01_CONTRACT_MATRIX.md` example references in framework skill docs. Followed by Liminara-side submodule bump + `sync.sh` re-run.
- **M-DOCS-02** (the reorg): five-commit sequence executing file moves (truth-model + shim-policy → `docs/governance/`; contract-matrix → `docs/architecture/indexes/`; PDFs → `docs/research/literature/`; delete the working proposal doc); rule-text articulation in `.ai-repo/rules/liminara.md` (new *Doc-tree boundaries* section + *Author-sequenced thinking convention* section); `artifact-layout.json` update; semantic adjustments to E-21 planning prose; retroactive `NN_lower_case.md` prefixing of `docs/research/` per git-creation order.
**Consequences:** Directory structure now expresses the register split that was implicit before (`docs/governance/` = bind-me artifact governance; `docs/architecture/`, `docs/decisions/`, `docs/research/`, `docs/history/`, `docs/analysis/` = inform-me). E-21's downstream artifacts (CUE schemas, fixtures, governance docs) land at paths that describe what they are. Frozen surfaces (`work/done/`, `work/agent-history/`, prior decisions) are not rewritten per D-2026-04-22-028 / D-2026-04-23-030 precedent; a redirect stub in `docs/governance/README.md` bridges 404 risk. The working proposal `work/proposals/docs-layout-reorg.md` is absorbed: reasoning → ADR-0003, plan → M-DOCS-02 spec, deliberation trail → M-DOCS-02 tracking doc; proposal doc itself deleted. `work/proposals/` is not established as a first-class artifact class. The `NN_` convention is formalised uniformly (numeric prefix + descriptor-case differs by directory — `UPPERCASE_WITH_UNDERSCORES` in architecture/analysis/brainstorm/domain_packs; `lower_case_with_underscores` in research). E-24's previously-planned separate `pack-contract-index.md` is dropped — the renamed contract matrix *is* the index.

## D-2026-04-24-032: `specsPath` is omitted from `artifact-layout.json`; framework removes it upstream
**Status:** active
**Context:** The `.ai/` framework defines three architect-skill output classes — research / architecture / specs — each with a corresponding config key in `artifact-layout.json` (`researchPath` / `architecturePath` / `specsPath`). Liminara's truth discipline (D-2026-04-04-022) holds that `docs/architecture/` contains only live or decided-next design prose. The third class ("specs") doesn't survive this discipline: pre-decided intent is research; decided intent is architecture; implementation scope is a milestone spec in `work/epics/`; Nygard reasoning is an ADR in `docs/decisions/`. There is no fourth state that needs its own directory. Each option for honoring `specsPath` in Liminara failed: value `docs/architecture/` dilutes the curated tree with pre-ratification architect drafts; value `docs/specs/` re-introduces the three-senses-of-spec ambiguity ADR-0003 is resolving; value `docs/architecture/specs/` still carries the ambiguity at subfolder level. Recorded as part of the E-22 review in the M-DOCS-02 tracking doc.
**Decision:** Liminara-side — `specsPath` is deliberately omitted from `.ai-repo/config/artifact-layout.json`. Framework-side — M-DOCS-01 ships a PR on the `ai-workflow` repo removing `specsPath` from `paths.md`, `sync.sh` defaults, the `architect` skill (collapsing its output split to two classes: research + architecture), the `planner` agent, and `migrate.sh §3.1e`. A new deprecation check in `migrate.sh` warns any consumer whose `artifact-layout.json` still carries `specsPath` that the key is removed and should be deleted.
**Consequences:** Until M-DOCS-01's framework PR merges, `migrate.sh §3.1e` continues to fire an informational note about `specsPath` being unset in Liminara — acceptable transient cost. Post-merge and submodule bump, the note stops firing and no consumer-side change is required (Liminara's config already omits the key). Other framework consumers with `specsPath` configured see the new deprecation warning and remove the key on their own cadence. ADR-0003 sub-decision 6 captures the full rationale and alternatives; this entry is the operational record of the Liminara-side omission + the framework removal commitment.

## D-2026-04-25-033: Liminara fixture-library converges on upstream `valid/invalid/` subdirectory split
**Status:** active
**Context:** M-PACK-A-01 (Contract-TDD tooling) was drafted before upstream framework PR #72 (issue [ai-workflow#37](https://github.com/23min/ai-workflow/issues/37)) landed today. PR #72 shipped `skills/design-contract.md` (tech-neutral), `docs/recipes/design-contract-cue.md` (first concrete recipe), and additive `templates/adr.md` `contract:` frontmatter fields. The upstream skill prescribes `<topic>/<version>/{valid,invalid}/<name>.<ext>` for the fixture library and treats invalid fixtures as a non-optional principle ("schema accepted something we didn't intend" is the dominant contract-bug class; only invalid fixtures catch it). Liminara's M-PACK-A-01 AC6 spec and the parent sub-epic *Schema-evolution check — specification* subsection had pinned a flatter `<topic>/fixtures/v<N>/<name>.yaml` shape with no valid/invalid distinction. The Liminara `scripts/cue-vet` no-arg loop (AC3/AC5, just shipped) treats every walked fixture as expected-to-validate — so a committed invalid fixture would be flagged as a regression on every commit, making invalid fixtures structurally impossible to ship in Liminara today.
**Decision:** Amend M-PACK-A-01 AC6 (and the matching parent sub-epic spec subsection) to add the `valid/invalid/` subdirectory split, converging on upstream's convention: `docs/schemas/<topic>/fixtures/v<N>/{valid,invalid}/<name>.yaml`. `scripts/cue-vet`'s no-arg loop is split into two sub-walks — `valid/` requires exit 0 per fixture, `invalid/` requires non-zero per fixture (an invalid fixture that *passes* `cue vet` is a regression, with a distinct failure-format string for that case). The schema-evolution loop walks `valid/` only (invalid fixtures aren't part of the forward-compat invariant). The pre-commit hook's fixture-stage regex tightens to require the `(valid|invalid)/` segment. Trigger now (not later): the fixture library is empty at this exact moment — M-PACK-A-02a lands the first schemas + fixtures — so the layout reshape costs zero in fixture-rewrite churn; reshaping after M-PACK-A-02a's first ADRs would be a multi-fixture migration.
**Consequences:** M-PACK-A-01 spec and `E-24-contract-design.md` *Schema-evolution check — specification* both edited for the new layout. `scripts/cue-vet`, `scripts/pre-commit-cue`, and the three test files (`test-cue-vet.sh`, `test-install-cue-hook.sh`, `test-schemas-layout.sh`) all refactored. `docs/schemas/README.md` updated. AC6 stays `done`; the reshape is recorded as a tracking-doc Work Log addendum on AC3/AC5/AC6, not a re-do. AC7's overlay file at `.ai-repo/skills/design-contract.md` becomes a smaller overlay than originally scoped (~70 lines vs. ~100), since the generic 7-step workflow now lives upstream. Upstream's `verify-contracts` skill + `contract-verify` Go binary (tracked at ai-workflow#69) and CI template will slot into Liminara zero-config when they land — Liminara's layout matches upstream's defaults exactly except for the top-level prefix (`docs/schemas/` vs. `docs/architecture/contracts/`, retained because `docs/architecture/contracts/` was removed in M-DOCS-02 per the E-22 doc-tree taxonomy work). Out of scope here: pulling the framework `.ai/` to upstream HEAD to get PR #72's deliverables on-disk, and adopting the upstream `templates/adr.md` `contract:` frontmatter fields in Liminara ADRs — both happen as part of the next routine framework sync, gated separately.

## D-2026-04-26-034: Migrate E-21 sub-epics to umbrella + peer-children pattern
**Status:** active (executed 2026-04-26 across commits 8dcff98, c070e32, 834ab53, 09ac615 plus the bundled cross-ref + graph-mutation commits; framework support landed via [ai-workflow#101](https://github.com/23min/ai-workflow/pull/101) phase 1 of [#99](https://github.com/23min/ai-workflow/issues/99))
**Supersedes (in part):** D-2026-04-22-029 — the prohibition on umbrella narratives is reversed; the prohibition on the sub-epic *shape* (nested specs, `composed_of` listings, parent-epic-with-milestones-alongside) is retained. The replacement pattern is umbrella + peer children connected by `parent:` frontmatter on the child epics.
**Context:** Two collisions surfaced on 2026-04-26 around E-21's sub-epic structure. (1) The wf-graph 0.8.0 `init --from-scan` resolver collapses `M-PACK-A-01`, `M-PACK-A-02a`, `M-PACK-A-02b`, `M-PACK-A-02c` into a single `M-PACK-A` node and flags it as a "complete vs. draft" status conflict recommending "archive the epic" — driven by the framework's `M-<TRACK>-<NN>` pattern not matching Liminara's `M-<TRACK>-<SUBTRACK>-<NN>[suffix]` convention. (2) The new 0.8.0 `add-milestone` verb (used by `plan-milestones` for sub-epic kickoff) is unverified against `M-PACK-B-01`-style IDs and may reject them, blocking E-25 plan-milestones when E-24 wraps. A design exploration covering tags-only (Option G), sub-epic subdirectories (Option B), peer epics with `depends_on` edges (Option E), and an Initiative kind (Option F) converged on a fifth design: an *umbrella epic* with *peer-epic children* connected by a `parent:` frontmatter field on each child. This preserves the unifying narrative D-029 was willing to sacrifice while avoiding the structural penalties of true sub-epics. The window for migration is narrow: M-PACK-A-01 wrapped 2026-04-25; M-PACK-A-02a hasn't started; the cost is roughly 2 hours of mechanical renaming. Mid-flight migration after M-PACK-A-02a starts at least doubles the cost.
**Decision:** *Conditional on framework PR landing*, transform E-21's sub-epic structure to umbrella + peer-children:
- **E-21** stays put as the umbrella; current `work/epics/E-21-pack-contribution-contract/epic.md` retains the unifying narrative (no content deleted, no relocation to `docs/analysis/`). E-21 becomes an epic with no milestones of its own, only child epics.
- **E-24 → E-21 child** (numbering: keep E-21 narrowed to contract-design? or new ID?). The active sub-epic with merged M-PACK-A-01 work becomes its own peer epic. Provisional new ID **E-24-contract-design**; `parent: E-21`. This avoids reusing `E-21` for both umbrella and a child (clearer mental model, even at the cost of more ID churn).
- **E-25 → E-25-runtime-pack-infrastructure**, `parent: E-21`, `depends_on: E-24`.
- **E-26 → E-26-pack-dx**, `parent: E-21`, `depends_on: E-25`.
- **E-27 → E-27-radar-extraction-and-migration**, `parent: E-21`, `depends_on: E-25` (admin-pack E-23 also feeds into the runtime work — captured as `E-25 depends_on E-23`).
- **Milestone IDs collapse to track-per-epic.** `M-PACK-A-01` → `M-CONTRACT-01`; `M-PACK-A-02a/b/c` → `M-CONTRACT-02/03/04`. Future `M-PACK-B-NN` becomes `M-RUNTIME-NN`; `M-PACK-C-NN` becomes `M-DX-NN`; `M-PACK-D-NN` becomes `M-RADAR-EXT-NN`. All conform to the framework `M-<TRACK>-<NN>` pattern.
- **Branch model.** Existing `epic/E-21-pack-contribution-contract` branch (which carries the M-PACK-A-01 merge) renames to `epic/E-24-contract-design`. Other peer-child branches cut from `main` when each becomes active. Umbrella has no branch.
- **External refs updated:** `work/decisions.md` (sub-epic ID rewrites), `work/gaps.md`, `CLAUDE.md` *Current Work* (full rewrite — sub-epic working rules section deleted), `work/agent-history/builder.md` (left as historical record per D-028/D-030 precedent), ROADMAP.md Phase 5c (rewrite), `docs/decisions/0003-doc-tree-taxonomy.md` (E-24 refs updated to E-24), milestone tracking docs relocate to `work/milestones/tracking/E-24-contract-design/`.
- **Graph mutation:** retire the existing E-21 sub-epic milestone IDs from `graph.yaml`; add E-24/25/26/27 as nodes with `parent: E-21` edges; rename milestone nodes; update edges. Done via wf-graph apply with a hand-authored patch.
- **Sequencing:** execute migration in a single PR, between M-PACK-A-01 wrap and M-PACK-A-02a kickoff. The framework PR (parent-epic support, drafted at `.ai-repo/scratch/E-21/framework-parent-epics-issue.md`) must land first, or Liminara must accept that the `parent:` frontmatter will be a documented-not-yet-honored field in the interim (acceptable transient cost — the wf-graph 0.5.0 resolver already ignores it).
- **Out of scope:** introducing an `Initiative` kind above the epic level; restructuring done epics (E-01..E-22) to fit the new pattern; renaming branch `epic/E-21-pack-contribution-contract` until immediately before the migration PR (avoids disrupting in-flight work).
**Consequences:** D-029's "sub-epics are an anti-pattern" rule narrows: the *shape* (nested under one parent dir, `composed_of:` listings, parent-epic-with-milestones-alongside) is still banned; the *concept* of a unifying epic-level narrative is allowed via `parent:` frontmatter on peer-epic children. ~10 files in `work/epics/` move and rename. The 35-line *Current Work* section in CLAUDE.md (and the temporary "E-21 sub-epic working rules" subsection) retire entirely. Audit warnings about unknown `parent:` parents stop firing once the framework PR lands. The wf-graph 0.8.0 milestone-ID-pattern collisions resolve as a side effect (track-per-epic IDs match `M-<TRACK>-<NN>` cleanly). The `E-25 depends_on E-23` edge (runtime depends on admin-pack) becomes an explicit graph edge that's currently buried in roadmap prose. M-PACK-A-01's merge commit `a2ceca3 Merge milestone/M-PACK-A-01` retains its original milestone ID in git history — historical commits are not rewritten, per the D-028/D-030 frozen-record precedent. Pre-rename PR titles, branch names, and external ref strings carrying "E-21" / "M-PACK-A" remain accurate at time of writing; readers cross-referencing post-migration will need a one-line note in the migration commit. Cost: ~2 hours of mechanical work in a single PR. Risk: if the framework PR is rejected or significantly altered, this decision needs revision before execution; the migration plan above assumes the proposed `parent:` frontmatter design is accepted upstream as drafted.

## D-2026-04-26-035: Execute D-034's deferred milestone rename; amend E-27 track to `RADX`
**Status:** active (executed 2026-04-26 in this PR)
**Amends (in part):** D-2026-04-26-034 — milestone-rename portion (D-034's "Milestone IDs collapse to track-per-epic" bullet) is executed here; track name for E-27 changes from `RADAR-EXT` (D-034) to `RADX` (single-segment, framework-conformant).
**Context:** D-034 specified the milestone rename `M-PACK-A-* → M-CONTRACT-*`, `M-PACK-B-* → M-RUNTIME-*`, `M-PACK-C-* → M-DX-*`, `M-PACK-D-* → M-RADAR-EXT-*` and sequenced its execution "in a single PR, between M-PACK-A-01 wrap and M-PACK-A-02a kickoff." D-034's other portions (sub-epic→peer-epic transformation, framework PR adoption, E-24 spec relocation) executed across commits 8dcff98 / c070e32 / 834ab53 / 09ac615 on 2026-04-26 morning. The milestone-rename portion was deferred and never run — the M-PACK-A-* IDs remained on disk, in `graph.yaml`, and in ~167 cross-doc citations. Inspection during the M-CONTRACT-02 kickoff conversation surfaced two issues: (1) D-034's `RADAR-EXT` track for E-27 has the same hyphenated-track problem as `PACK-A` that D-034 was fixing — `M-RADAR-EXT-NN` violates the framework's single-segment-track rule; (2) the existing repo precedent (17 of 18 milestone tracks: `RAD`, `CS`, `DOCS`, `EE`, `OBS`, `OTP`, `PORT`, `TRUTH`, `WARN`, etc.) is uniformly single-segment thematic, against which `PACK-A/B/C/D` was the lone outlier. D-034's CONTRACT/RUNTIME/DX selection follows the precedent; RADAR-EXT does not.
**Decision:** Execute D-034's milestone rename today, with the single amendment that E-27's track is `RADX` (concise, single-segment, signals "second pass on Radar" — distinct from the original `RAD` track of E-11/E-11b). Specifically:
- Real renames (4 milestones in E-24): `M-PACK-A-01 → M-CONTRACT-01`, `M-PACK-A-02a → M-CONTRACT-02`, `M-PACK-A-02b → M-CONTRACT-03`, `M-PACK-A-02c → M-CONTRACT-04`. Letter suffixes collapse to flat numbering; the "originally one milestone, split into three" semantic survives via consecutive numbers + milestone titles.
- Forward-ref renames (no milestones materialized; only citations in specs/rules/gaps): `M-PACK-B-01a → M-RUNTIME-01`, `M-PACK-B-01b → M-RUNTIME-02`, `M-PACK-B-02 → M-RUNTIME-03`, `M-PACK-B-03 → M-RUNTIME-04`. `M-PACK-C-01..03 → M-DX-01..03`. `M-PACK-D-01a → M-RADX-01`, `M-PACK-D-01b → M-RADX-02`, `M-PACK-D-02 → M-RADX-03`.
- Spec files renamed in place via `wf-graph rename` (atomic graph-node + spec-filename update). Tracking doc renamed via `git mv`. Cross-doc citations updated by single-pass sed across in-scope files (excluding `work/decisions.md` historical entries, `work/agent-history/`, `work/done/`, the `.ai/` framework submodule, and `.ai-repo/scratch/`). Two test-script regexes updated from `M-PACK-[A-Z]-` to `M-(CONTRACT|RUNTIME|DX|RADX)-`. Two unsplit-reference sentences (E-25 epic, E-27 epic) rephrased — the "M-PACK-B-01 is split into 01a/01b" framing no longer makes sense after flat-numbering, replaced with "originally planned as one milestone, separated at plan-time per the 2026-04-23 ultrareview sizing analysis."
- Pre-existing drift incidentally fixed: `.ai-repo/rules/contract-design.md` and `.ai-repo/skills/design-contract.md` referenced the stale path `work/epics/E-21-pack-contribution-contract/E-24-contract-design.md` (from before D-034's epic-promotion); both now reference `work/epics/E-24-contract-design/epic.md`. The matching test-script grep patterns also updated.
- Branch model unchanged from D-034 — `epic/E-24-contract-design` carries this rename; future milestone branches cut as `milestone/M-CONTRACT-NN`. The merged `milestone/M-PACK-A-01` branch is left as-is (frozen historical record per D-028/D-030/D-034 precedent).
**Consequences:** Zero `M-PACK-` strings remain in the live tree — the outlier track is fully retired. All seven `scripts/tests/test-*.sh` pass. `wf-graph validate` is clean (zero new findings). The `RADX` track is one new project rule alongside `CONTRACT`/`RUNTIME`/`DX` for E-25/E-26/E-27 milestone authoring; no E-25/E-26/E-27 milestones exist yet so those tracks have no real graph nodes until each epic's `plan-milestones` runs. Pre-rename git history (the merged M-PACK-A-01 branch, its merge commit, PR titles, Slack mentions carrying "M-PACK-A") remains historically accurate; readers cross-referencing through this rename event read this entry. Cost: ~40 minutes of mechanical work + hand-fix sentences. Risk: low — no IDs were preserved/aliased (no shim layer), the rename is final.
