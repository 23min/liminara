...
3. **Local invocation: a single, named entry point**
   ...
   - The entry point exits non-zero on any `cue vet` failure and prints the failing fixture path + topic schema + CUE error message in the failure-semantics format specified in the parent sub-epic's "Schema-evolution check — specification" subsection (see `work/epics/E-21-pack-contribution-contract/E-24-contract-design.md`).
...
8. **Liminara-local `contract-design` rule exists at `.ai-repo/rules/contract-design.md`**
   ...
   - The rule references — by file path — the contract-matrix index at `docs/architecture/indexes/contract-matrix.md` and the parent sub-epic spec at `work/epics/E-21-pack-contribution-contract/E-24-contract-design.md`, so reviewer agents can chase citations without re-deriving them.
...
- Parent sub-epic spec: `work/epics/E-21-pack-contribution-contract/E-24-contract-design.md`
...
- Reference milestone spec format: `work/done/E-19/M-WARN-01-runtime-warning-contract.md`
