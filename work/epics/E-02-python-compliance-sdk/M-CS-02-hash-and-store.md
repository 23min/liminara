...
- [ ] `hash.py`: SHA-256 hashing produces `sha256:{64 lowercase hex}` encoding
- [ ] `hash.py`: canonical JSON serialization matches RFC 8785 (sorted keys, no whitespace, UTF-8)
- [ ] `hash.py`: `hash_bytes(raw_bytes)` → artifact hash
- [ ] `hash.py`: `hash_event(event_type, payload, prev_hash, timestamp)` → event hash (excludes event_hash itself from input)
- [ ] `artifact_store.py`: write blob → returns hash, stored at `{store_root}/{hash[0:2]}/{hash[2:4]}/{hash}`
- [ ] `artifact_store.py`: write is idempotent (same content → same hash → skip if exists)
- [ ] `artifact_store.py`: read by hash → returns raw bytes
- [ ] `event_log.py`: append event → writes one canonical JSON line to `{runs_root}/{run_id}/events.jsonl`
- [ ] `event_log.py`: each event contains `event_hash`, `event_type`, `payload`, `prev_hash`, `timestamp`
- [ ] `event_log.py`: first event has `prev_hash: null`
- [ ] `event_log.py`: each subsequent event's `prev_hash` equals previous event's `event_hash`
- [ ] `event_log.py`: read all events from a run → returns list of event dicts
- [ ] `event_log.py`: verify hash chain → returns (valid: bool, error_detail: str | None)
- [ ] `decision_store.py`: write decision record → canonical JSON at `{runs_root}/{run_id}/decisions/{node_id}.json`
- [ ] `decision_store.py`: decision_hash computed over all fields except decision_hash itself
