...
E-21b declares one compatibility shim under `docs/governance/shim-policy.md`. This section is the sub-epic-level shim record; **M-PACK-B-01b's spec (the milestone where the shim file lands) must carry its own `## Compatibility shims` section with the same record** when authored (spec review blocks on absence), per the policy's "owning milestone spec names the shim explicitly" rule.
