...
What does NOT land in E-21b: the Python SDK, `liminara_widgets` widgets, CLIs, test harness, the file-watch demo pack (all E-21c), and the extraction of Radar to a submodule (E-21d).
...
**M-PACK-B-01b — PackLoader + PackRegistry + Radar through the pipeline:**
...
- [ ] **Admin-pack-shape proxy pack — shape validation (loaded in B-01b, executed in B-03).** An in-tree example pack at `examples/admin_shape_proxy/` (or equivalent; exact path fixed by M-PACK-B-01b) with **three plan entrypoints** (intake / reconcile / export, shape-borrowed from admin-pack's documented three-workflow structure in `admin-pack/v2/docs/architecture/bookkeeping-pack-on-liminara.md`), **three trigger declarations** in its manifest (`:file_watch` intake, `:cron` reconcile, `:manual` export), declared pack-instance state, declared (mock) secrets. **M-PACK-B-01b validates shape only**: `PackLoader` validates the manifest against CUE, `PackRegistry.get/1` returns a pack with three plans and three trigger declarations; the proxy's *manifest shape* exercises the contract from a second live source. **Dispatch (actually firing each trigger and running each plan end-to-end) lands in M-PACK-B-03** when `TriggerManager` exists. **Explicitly not admin-pack** — no bookkeeping domain, no real PDFs, no real GL codes; this fixture exercises the contract's *shape pressures* alone while admin-pack's domain work remains E-22. Rationale: E-21's forcing-function argument (`roadmap.md:94`) requires a second live pack during E-21 — this fixture provides it without pulling admin-pack's domain work forward (see E-21a's Admin-pack citation discipline success criterion for the paired mechanism).
...
**M-PACK-B-02 — SurfaceRenderer + SecretSource + A2UI MultiProvider:**
- (The SurfaceRenderer / SecretSource / MultiProvider criteria in this section are owned by M-PACK-B-02.)

**M-PACK-B-03 — TriggerManager + FSScope + FS-scope enforcement + proxy execution:**
...
1. **Radar is the live validator.** If any M-PACK-B-* milestone breaks Radar, the milestone is not done. The staged-loading approach (namespace reorg + boundary → loader + registry + Radar-through-pipeline → surface renderer → trigger manager + FSScope + proxy execution) means each milestone tests against Radar incrementally. The B-01 split (a/b) separates "Pack.API shape" from "runtime pack-loading" so each has its own green signal; this matches the repo's milestone cadence (see E-19 / E-20 precedents).
2. **Generated manifest for Radar is a declared compatibility shim** under `docs/architecture/contracts/02_SHIM_POLICY.md`. It exists in-tree during E-21b to prove the pack-loader works. E-21d replaces it with Radar's own canonical authored manifest when Radar moves to its submodule. Full shim record: see "Compatibility shims" section below.
...
|---|---|---|
| Radar's existing code has implicit assumptions that break when loaded through the generic pack-loader | High | M-PACK-B-01b lands with Radar loading through the new path. Every subsequent milestone re-runs Radar e2e. Issues surface immediately, not at E-21d. |
...
E-21b declares one compatibility shim under `docs/architecture/contracts/02_SHIM_POLICY.md`. This section is the sub-epic-level shim record; **M-PACK-B-01b's spec (the milestone where the shim file lands) must carry its own `## Compatibility shims` section with the same record** when authored (spec review blocks on absence), per the policy's "owning milestone spec names the shim explicitly" rule.

### Radar generated `pack.yaml` — temporary in-tree scaffolding

- **Identity.** A generated `pack.yaml` for Radar lives in-tree (path fixed by M-PACK-B-01b implementation; suggested `runtime/apps/liminara_radar/pack.yaml`). It expresses Radar's *current* shape in the ADR-MANIFEST-01 schema so `PackLoader` can load Radar through the generic code path.
- **Why the shim is allowed.** Without it, Radar dies the moment `PackRegistry` requires manifests; the alternative is a big-bang Radar extraction in one milestone — exactly what E-21d avoids by staging. It adapts shape, not semantics — Radar's execution, outputs, and artifacts are identical whether loaded through the old path or via this manifest.
- **Removal trigger.** E-21d M-PACK-D-01b replaces this with `radar-pack`'s own authored canonical `pack.yaml` when Radar moves to its submodule. The shim file is deleted in that same milestone.
- **Required file header comment.** The generated file's first lines must be a comment block that identifies it as a shim so anyone reading the raw file sees it. Minimum format:
  ...
  ```yaml
  # SHIM — E-21b Radar generated manifest.
  # Removal trigger: E-21d M-PACK-D-01b (Radar extraction to radar-pack submodule).
  # Policy: docs/architecture/contracts/02_SHIM_POLICY.md
  # Gap: work/gaps.md — "Radar generated pack.yaml shim"
  ```

  M-PACK-B-01b's reviewer verifies this header is present before approving the PR that lands the file.
- **`work/gaps.md` entry.** The shim survives beyond its owning milestone (M-PACK-B-01b); it lives through M-PACK-B-02, M-PACK-B-03, E-21c, and into the start of E-21d. That triggers the policy's "gap entry" requirement. A `work/gaps.md` entry is added as part of M-PACK-B-01b's merge and names E-21d M-PACK-D-01b as the removal trigger.
- **Generation mechanism.** Whether the file is emitted by a tiny generator script, hand-authored-once-and-maintained, or regenerated from Radar's current shape at test boot is an implementation choice left to M-PACK-B-01b. Whichever mechanism is chosen, the file that `PackLoader` consumes must be the version that carries the SHIM header and lives at the path M-PACK-B-01b declares — no invisible in-memory-only generation.
...
- **No dependency on E-21c.** E-21b validates against Radar and the admin-pack-shape proxy pack; it does not require the Python SDK or external-author tooling.
