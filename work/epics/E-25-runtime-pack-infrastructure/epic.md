...
|---|---|---|
| Radar's existing code has implicit assumptions that break when loaded through the generic pack-loader | High | M-PACK-B-01b lands with Radar loading through the new path. Every subsequent milestone re-runs Radar e2e. Issues surface immediately, not at E-27. |
...
E-25 declares one compatibility shim under `docs/governance/shim-policy.md`. This section is the sub-epic-level shim record; **M-PACK-B-01b's spec (the milestone where the shim file lands) must carry its own `## Compatibility shims` section with the same record** when authored (spec review blocks on absence), per the policy's "owning milestone spec names the shim explicitly" rule.
...
- **Removal trigger.** E-27 M-PACK-D-01b replaces this with `radar-pack`'s own authored canonical `pack.yaml` when Radar moves to its submodule. The shim file is deleted in that same milestone.
- **Required file header comment.** The generated file's first lines must be a comment block that identifies it as a shim so anyone reading the raw file sees it. Minimum format:
  ...
  # Removal trigger: E-27 M-PACK-D-01b (Radar extraction to radar-pack submodule).
...
- **`work/gaps.md` entry.** The shim survives beyond its owning milestone (M-PACK-B-01b); it lives through M-PACK-B-02, M-PACK-B-03, E-26, and into the start of E-27. That triggers the policy's "gap entry" requirement. A `work/gaps.md` entry is added as part of M-PACK-B-01b's merge and names E-27 M-PACK-D-01b as the removal trigger.
