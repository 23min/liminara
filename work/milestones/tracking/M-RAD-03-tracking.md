...
- [x] AC3: Radar.Ops.Summarize (Python, recordable) — Haiku per-cluster summaries, decisions recorded
  - KNOWN: multi-decision replay broken at runtime level (Decision.Store overwrites per node_id) — E-11c
- [x] AC4: Radar.Ops.ComposeBriefing (Elixir, pure) — assemble briefing data structure
  - KNOWN: `run_id` is a plan-time ID (`radar-YYYYMMDDTHHMMSS`), not the runtime's actual run_id. Consistent between dedup and compose, but not the real execution run_id. Threading real run_id through plan execution is E-11c scope.
...
## Known Limitations (deferred to E-11c / Phase 5a)
