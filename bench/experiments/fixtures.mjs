...
// 2. Internal models — dag-map's own test corpus (with provided routes)
// 3. External North DAGs — pure topology (routes auto-discovered)
...
export async function loadExperimentFixtures() {
  ...
  // Tier 2: Internal models (8+ nodes, with routes)
  const internal = models
    .filter(m => m.dag.nodes.length >= 8)
    .map(m => ({ ...m, source: 'internal' }));
  fixtures.push(...internal);

  // Tier 3: External North DAGs (15-50 nodes, routes auto-discovered)
