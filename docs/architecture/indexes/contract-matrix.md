...
| Warning and degraded-success contract | `runtime/apps/liminara_core/lib/liminara/warning.ex`, `runtime/apps/liminara_core/lib/liminara/op_result.ex`, `runtime/apps/liminara_core/lib/liminara/run.ex` (`Run.Result`), `runtime/apps/liminara_core/lib/liminara/run/server.ex` (`warning_payload/1` + `run_partial` terminal event emission) | `work/epics/E-21-pack-contribution-contract/E-24-contract-design.md` (ADR-OPSPEC-01 will codify warning shape + the `run_completed`/`run_partial`/`run_failed` terminal event taxonomy in canonical CUE form); decisions D-2026-04-20-025 (run_partial terminal event) and D-2026-04-20-026 (no backward-compat shims for in-flight contract fixes) | `work/done/E-19/epic.md`, `work/done/E-20-execution-truth/M-TRUTH-01-execution-spec-outcome-design.md`, `work/done/E-20-execution-truth/M-TRUTH-02-core-runtime-contract-migration.md` | Warnings are first-class: terminal event type is 1:1 with `Run.Result.status` (no payload-field discriminator); no UI-only or decision-shaped warning side channel; no production fallback clauses that accept both legacy and spec-conformant fixture shapes |
