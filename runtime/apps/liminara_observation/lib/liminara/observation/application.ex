defmodule Liminara.Observation.Application do
  ...
  def start(_type, _args) do
    ...
    case Supervisor.start_child(sup, {A2UI.Server, provider: Liminara.Observation.A2UISocketProvider, port: port, ip: {0, 0, 0, 0}}) do
      {:ok, _} ->
        Logger.info("A2UI endpoint running on port #{port}")

      {:error, reason} ->
        Logger.warning("A2UI endpoint failed to start: #{inspect(reason)} — continuing without it")
    end
