...
**Active focus:** M-PACK-A-01 — Liminara-project Contract-TDD tooling: ship the harness (CUE in devcontainer pinned via repo-root `.tool-versions`, `scripts/cue-vet` entry point, pre-commit hook installer, schema-evolution loop, `docs/schemas/` layout placeholder, Liminara-local `.ai-repo/skills/design-contract.md` + `.ai-repo/rules/contract-design.md`) so M-PACK-A-02a can land its first ADRs into a working tree. Sits inside E-21 → E-21a (Contract Design); other three E-21a milestone specs (M-PACK-A-02a/b/c) remain at status `draft`.
**Why now:** No contract authoring can start until `.cue` files validate and fixtures walk. E-19 (warning/degraded-outcome contract) is merged on `main`; ADR-OPSPEC-01 in M-PACK-A-02a will codify it once this harness lands. E-21 ships before E-12 (Op Sandbox) per Phase 5c sequencing decided 2026-04-25.
**Active branch:** `milestone/M-PACK-A-01` (cut from `epic/E-21-pack-contribution-contract` on 2026-04-25; epic branch is the only epic-level branch — no `epic/E-21a-...`, see *E-21 sub-epic working rules* below).
**Next action:** Implement AC1 first (`.tool-versions` at repo root) via `/wf-tdd-cycle`, then AC2 → AC8 in order. Spec is locked on most decisions in *Design Notes*; builder pauses and asks if any constraint surfaces that demands a change.
