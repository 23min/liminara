...
**Active focus:** **M-CONTRACT-03 — Packs-as-running-systems (5 ADRs)** is the next milestone, newly unblocked by the M-CONTRACT-02 wrap. It ships ADR-SURFACE-01 (UI surfaces), ADR-TRIGGER-01 (triggers), ADR-FILEWATCH-01 (file-watch semantics), ADR-FSSCOPE-01 (filesystem scope), and ADR-SECRETS-01 (secrets) — each paired with CUE schema + valid/invalid fixtures + worked example + named reference implementation, following the M-CONTRACT-02 bundle pattern. Owning epic is **E-24 (Pack Contract Design)**, child of umbrella **E-21**. Status: `draft` (not yet started; spec is authored).
**Why now:** M-CONTRACT-02 merged 2026-05-02 and pushed (commit `b1ffb24` + the doc-lint regen `6728a3c`). M-CONTRACT-03's ADRs lock the contract surface that E-25 (M-RUNTIME-02 + M-RUNTIME-04) needs to build against — `SurfaceRenderer`, `TriggerManager`, the file-watch loop, FS-scope enforcement, `SecretSource`. Running in parallel: M-CONTRACT-04 is also unblocked but not yet next-up (it's the heavier end-of-E-24 milestone covering MULTIPLAN/EXECUTOR/EVOLUTION/CONTENT/LAYOUT/BOUNDARY/REGISTRY/LA + the dynamic-pipelining design captured at D-2026-05-02-039).
**Active branch:** `epic/E-24-contract-design` (post-M-CONTRACT-02 merge). The next milestone branch `milestone/M-CONTRACT-03` will be cut when work begins. Spec: `work/epics/E-24-contract-design/M-CONTRACT-03-packs-as-running-systems.md`.

**M-CONTRACT-02 (just wrapped) — recap:** 5 ADRs (`docs/decisions/0004-0008`), 5 schemas + 52 fixtures (`docs/schemas/{op-execution-spec,wire-protocol,replay-protocol,manifest,plan}/`), 2 RFCs (`docs/architecture/proposals/{dynamic-pipelining-via-contract-routing,pipeline-scoped-run-context}.md`), spec amendment for worked-example abbreviation, four decisions ratified (D-036/037/038/039), four gap entries filed, contract-matrix updated with 5 new rows. `./scripts/cue-vet` clean. wf-doc-lint full re-ran post-merge (102 docs / 806 sections). Tools promoted: `scripts/reflow-md.py` + `scripts/detect-hardwrap-md.py`.
